...
export default (function (window, document) {
    ...
    class App {
        ...
        attachView() {
            ...
            this.$chatFrame = widget.querySelector(`.${STYLE_NAMESPACE}-chat-container>iframe`)
        ...
        getChatAppUrl(path = '') {
            //console.log("hello", this.options.app_key)
            // const urlPath = path ? `/${path}` : ''
            return `${process.env.JEEVES_CHAT_APP_URL}/${this.options.app_key}`
        }
        loadNotificationFrame() {
            ...
            this.$chatCloser.addEventListener('click', this.onNotificationToggleClicked.bind(this))

            if (!this.options.useNotificationframe) return

            // this.$notificationFrame.src = this.getChatAppUrl() + '/notifications'
            this.$notificationFrame.onload = this.onNotificationFrameLoaded.bind(this)
        }
        ...
        configureAutoOpen() {
            if (this.options.autoOpen !== true || !this.settings.autoOpenEnabled) return

            const time = (isNaN(this.options.timeToOpen) ? 10 : this.options.timeToOpen) * 1000
            this.autoOpenHandler = setTimeout(this.onNotificationToggleClicked.bind(this), time)
        }
        ...
        onNotificationToggleClicked() {
            ...
            const open = this.$chatFrame.parentNode.className.split(' ').includes('open')
            ...
            if (open) {
                ...
                if (this.options.loadAfterWidgetOpen)
                    setTimeout(this.loadChatFrame.bind(this), this.options.delayBeforeLoad)
        ...
        onChatFrameLoaded() {
            ...
            this.$chatFrame.parentNode.classList.add(`${STYLE_NAMESPACE}-chat-loaded`)
            this.$chatFrame.onload = null
        ...
        setupChannelWithChatApp() {
            window.addEventListener('message', this.onChatAppSignalReceived.bind(this), false)
