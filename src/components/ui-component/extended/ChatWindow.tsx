...
const chat = require('constant/chat.json');
...
const ChatWindow = ({
  ...
  const agent = useSelector(state => state.agent.agentMode);
  ...
  const handleUpdateAgentMode = (event: React.ChangeEvent<HTMLInputElement>) => {
    updateAgentMode(event.target.checked ? PAUSE_MODE : AUTOMATIC_MODE);
  };
            ...
            <PopIn delay={0.8}>
              <ChatMessage
                message={{
                  ...
                  value: "👉 " + "Create an agent by adding a name / goal, and hitting deploy! Try our examples below!",
                }}
              />
            </PopIn>
            <PopIn delay={1.5}>
    ...
    >
      {message.type != MESSAGE_TYPE_SYSTEM && (
        ...
        <>
          ...
          <span className="mr-2 font-bold">{chat[getMessagePrefix(message)]}</span>
        </>
      )}

      {message.type == MESSAGE_TYPE_THINKING && (
        <span className="italic text-zinc-400">
          {`${chat["RESTART_IF_IT_TAKES_X_SEC"]}`}
        </span>
      )}
