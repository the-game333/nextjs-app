...
const Input = (props: InputProps) => {
  const { small, placeholder, left, value, type, onChange, setValue, disabled, attributes, inputRef, toolTipProperties, onKeyDown } = props;
    ...
    inputElement = (
      <Combobox
        ...
        styleClass={{
          ...
          option: 'cursor-pointer px-2 py-2 font-mono text-sm text-white/75 hover:bg-blue-500 sm:py-3 md:text-lg'
        }}
      />
    );
    ...
    inputElement = (
      ...
      >
        <input
          ...
          {...attributes}
        />
        <div className="flex-none cursor-pointer rounded-full p-2 hover:bg-white/20" onClick={(e) => handleApiKeyToggle()}>
  ...
  return (
    ...
    >
      ...
      {inputElement}
      {isTypeRange() && <p className="m-auto mx-4 w-1/6 px-0 text-center text-sm md:text-lg">{value}</p>}
    </div>
  );
};
