import type { RequestBody } from "utils/agent/interfaces";
...
export class AgentApi {
  ...
  async analyzeTask(task: string): Promise<Analysis> {
    return await this.post<Analysis>("/api/agent/analyze", {
      task: task,
      toolNames: useSelector(state => state.agenttool.tools.map((tool: any) => tool.name)),
    });
  }
  ...
  private async post<T>(url: string, data: Omit<RequestBody, "goal" | "modelSettings">) {
    ...
    try {
      return (await axios.post( BASE_URL + url, requestBody)).data as T;
    } catch (e) {
      this.onError(e);
    }
  }
